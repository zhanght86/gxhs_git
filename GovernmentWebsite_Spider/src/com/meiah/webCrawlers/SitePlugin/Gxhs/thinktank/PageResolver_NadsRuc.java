 ...
 * @date 2015-10-7
 */
public class PageResolver_NadsRuc extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_NadsRuc.class);
	public PageResolver_NadsRuc(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_content().contains("网页建设中")) {
				ne = null;
				return ne;
			}
		}
