 ...
 * @date 2015-10-7
 */
public class PageResolver_Cqass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Cqass.class);
	public PageResolver_Cqass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		ne.setNews_content(content.replaceAll("【】 【】 【】","").replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", ""));
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().contains("通知") || ne.getNews_title().length() <= 7) {
				ne = null;
				return ne;
			}
		}
