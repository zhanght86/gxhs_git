 ...
 * @date 2015-10-7
 */
public class PageResolver_Sjzsk extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Sjzsk.class);
	public PageResolver_Sjzsk(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 || ne.getNews_title().contains("下载")) {
				ne = null;
				return ne;
			}
		}
