 ...
 * @date 2015-10-7
 */
public class PageResolver_Iple_Cass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Iple_Cass.class);
	public PageResolver_Iple_Cass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(newsAuthor.contains("文章作者：")) {
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者：") + 3,newsAuthor.indexOf(" "));
		} else {
			newsAuthor = "";
		}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(ne.getNews_title().endsWith("研究室") || ne.getNews_title().endsWith("名单") || ne.getNews_title().endsWith("方案")
					|| ne.getNews_title().endsWith("公告") || ne.getNews_title().endsWith("计划") || ne.getNews_title().endsWith("情况表	")
					|| ne.getNews_title().endsWith("名录") || ne.getNews_title().contains("招生简章") || StringUtils.isEmpty(ne.getNews_content())) {
				ne = null;
				return ne;
			}
		}
