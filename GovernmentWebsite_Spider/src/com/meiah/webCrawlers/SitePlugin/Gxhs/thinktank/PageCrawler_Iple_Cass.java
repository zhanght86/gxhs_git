...
public class PageCrawler_Iple_Cass extends PageCrawler {
	
	public static final String TODAY = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	public PageCrawler_Iple_Cass(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected ArrayList<TaskLink> extractTaskLinks(String webContent, Task task, TaskLink link) {
		...
		for (int i = 0; i < links.size(); i++) {
			...
			if(!(tl.getUrl().equals("http://iple.cass.cn/cate/27.htm"))) {
				urlList.add(tl);
			}
		}
		return urlList;
