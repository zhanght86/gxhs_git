...
/**
 * 教育部高等学校社会科学发展研究中心，正文解析获取作者
 * 
 * @author dingrr
 * @date 2015-10-4
 */
public class PageResolver_Crifs extends PageResolver_General{

	public PageResolver_Crifs(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replace("来源：", ""));
		...
		}else if(newsAuthor.contains("编辑")) {
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("编辑"));
			newsAuthor = newsAuthor.replace("编辑：", "");
		}else {
			newsAuthor = "";
		}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().contains("简介") || ne.getNews_content().contains("点此下载本视频")
					|| ne.getNews_title().contains("简章") || ne.getNews_title().contains("招生")) {
				ne = null;
				return ne;
			}
		}
