 ...
 * @date 2015-10-7
 */
public class PageResolver_Jds_Cass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Jds_Cass.class);
	public PageResolver_Jds_Cass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("文章来源：")) {
				newsSource = newsSource.substring(newsSource.indexOf("文章来源：") + 5);
				if(newsSource.contains("点击数")) {
					newsSource = newsSource.substring(0,  newsSource.indexOf("点击数"));
				}
			}
		...
		if(newsAuthor.contains("作者")) {
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("：") + 1,newsAuthor.indexOf(" "));
		} else {
			newsAuthor = "";
		}
	...
	public static void main(String[] args) {
		String newsSource = "作者：网站编辑部    文章来源：本站原创    点击数：";
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("文章来源：")) {
				newsSource = newsSource.substring(newsSource.indexOf("文章来源") + 5);
				if(newsSource.contains("点击数")) {
