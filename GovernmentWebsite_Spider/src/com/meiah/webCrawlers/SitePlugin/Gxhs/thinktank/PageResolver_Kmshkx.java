 ...
 * @date 2015-10-7
 */
public class PageResolver_Kmshkx extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Kmshkx.class);
	public PageResolver_Kmshkx(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(ne.getNews_title().contains("联系我们")|| ne.getNews_title().contains("招聘") || ne.getNews_content().endsWith(".doc") || ne.getNews_content().endsWith(".xls")
					|| ne.getNews_title().length() <= 3 || StringUtils.isEmpty(ne.getNews_content())) {
				ne = null;
				return ne;
			}
		}
