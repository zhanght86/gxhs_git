 ...
 * @date 2015-10-9
 */
public class PageResolver_ThinkTank_pageTurnContent extends
		PageResolver_General {

	public PageResolver_ThinkTank_pageTurnContent(Task task) {
		super(task);
	}
...
//		ne.setSpot_code(Config.getSpotID());
		String domain = ne.getWebsite_url().substring(ne.getPage_url().indexOf("://") + 3);
...
//						System.out.println("分页链接：" + aHrefLink);
		...
		if (alreadyPageTurn == 0) {// 0代表没有分页
			...
			//把正文中的网络编辑删除
			if(content.contains("（网络编辑")) {
				String ss = content.substring(content.indexOf("（网络编辑"),content.lastIndexOf("）") + 1);
				content = content.replaceAll(ss, "");
			}
			String replace = "";
			...
			content = content.replaceAll("(?is)<(no)?script.*?((/>)|(</(no)?script>))","").replaceAll(replace, "");
			...
			//把正文中的网络编辑删除
			if(turnPageConent.contains("（网络编辑")) {
				String ss = turnPageConent.substring(turnPageConent.indexOf("（网络编辑"),turnPageConent.lastIndexOf("）") + 1);
				turnPageConent = turnPageConent.replaceAll(ss, "");
			}
			String replace = "";
			if(content.contains("<?xml:namespace")) {
				replace = content.substring(content.indexOf("<?xml:namespace"));
			...
			turnPageConent = turnPageConent.replaceAll("(?is)<(no)?script.*?((/>)|(</(no)?script>))","").replace(replace, "");
