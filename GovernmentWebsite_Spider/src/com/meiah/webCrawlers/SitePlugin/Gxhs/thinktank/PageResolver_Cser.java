...
/**
 * 解析时，获取中国经济体制改革研究会正确的作者信息
 * @author dingrr
 * @date 2015-10-7
 */
public class PageResolver_Cser extends PageResolver_General {
	protected Logger logger = Logger
			.getLogger(PageResolver_Cser.class);

	public PageResolver_Cser(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if (newsAuthor.contains("|")) {
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("|") + 1);
		} else {
			newsAuthor = "";
		}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 || ne.getNews_content().contains("对本文发表看法，请电邮") ) {
				ne = null;
				return ne;
			}
		}
