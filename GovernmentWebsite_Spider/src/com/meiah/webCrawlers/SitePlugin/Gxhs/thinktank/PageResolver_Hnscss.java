 ...
 * @date 2015-10-7
 */
public class PageResolver_Hnscss extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Hnscss.class);
	public PageResolver_Hnscss(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) 
					|| ne.getNews_title().contains("通知") || ne.getNews_title().length() <= 6) {
				ne = null;
				return ne;
			}
		}
