 ...
 * @date 2015-10-7
 */
public class PageResolver_Ourhkfoundation extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Ourhkfoundation.class);
	public PageResolver_Ourhkfoundation(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		String newsAuthor = "";
		if(content.contains("撰文：")) {
			newsAuthor = content.substring(content.indexOf("撰文："));
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("：") + 1 , newsAuthor.indexOf("　"));
		} else {
			newsAuthor = getNewsAuthor(task.getSiteConfig(), page, doc);
		}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content())) {
				ne = null;
				return ne;
			}
		}
