...
/**
 * 解析时，获取国家信息中心网站的正确作者信息及图片链接地址
 * @author dingrr
 * @date 2015-10-7
 */
public class PageResolver_FuDan extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_FuDan.class);
	public PageResolver_FuDan(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		//过滤正文中无用的信息
		if(content.contains("本页最后修改时间")) {
			String isContains = content.substring(content.indexOf("本页最后修改时间"));
			content = content.replaceAll(isContains, "");
		}
		String replace = "";
		...
		if(null != newsAuthor && !newsAuthor.equals("")) {
