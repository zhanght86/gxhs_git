...
public class PageCrawler_Jgswh extends PageCrawler {
	
	public static final String TODAY = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	public PageCrawler_Jgswh(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected String getPageContent() {
		...
		if(webContent.contains("文章显示")) {
			webContent = webContent.substring(webContent.indexOf("文章显示") , webContent.indexOf("底部"));
		}
		return webContent;
	}
}
