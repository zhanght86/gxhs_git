 ...
 * @date 2015-10-7
 */
public class PageResolver_Fjwsg extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Fjwsg.class);
	public PageResolver_Fjwsg(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(ne.getNews_title().contains("（")) {
			preTitle = newsTitle.substring(0 , newsTitle.indexOf("（"));
		}
		if(preTitle.length() <= 3 || StringUtils.isEmpty(ne.getNews_content())) {
			ne = null;
			return ne;
		}
