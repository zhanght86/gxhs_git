 ...
 * @date 2015-10-7
 */
public class PageResolver_Jlass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Jlass.class);
	public PageResolver_Jlass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(ne.getNews_title().length() <= 6 || ne.getNews_title().contains("成员") || ne.getNews_title().contains("简介") || ne.getNews_title().contains("研究所")
