 ...
 * @date 2015-10-7
 */
public class PageResolver_NiisCass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_NiisCass.class);
	public PageResolver_NiisCass(Task task) {
		super(task);
	}
...
//		ne.setSpot_code(Config.getSpotID());
		ne.setWebsite_domain(SiteCrawler.topDomain);
		ne.setWebsite_url(link.getRefererUrl());
...
//			ne.setIsHomePageNews(1);
		...
		if(newsAuthor.contains("作者：")) {
			newsAuthor = newsAuthor.substring(newsAuthor.indexOf("：") + 1,newsAuthor.indexOf("来源"));
		} else {
			newsAuthor = "";
		}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 4) {
				ne = null;
				return ne;
			}
		}
