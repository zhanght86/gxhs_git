...
import com.meiah.util.NanyangWebPageDownloader;
...
public class PageCrawler_NanyangGuangBo extends PageCrawler{
	private static Logger logger = Logger.getLogger(PageCrawler_NanyangGuangBo.class);
	public PageCrawler_NanyangGuangBo(TaskLink link, Task task) {
		super(link, task);
	}
	/**
	...
	protected String getPageContent() {
		...
		NanyangWebPageDownloader downloader = new NanyangWebPageDownloader(downloadUrl);
	...
	protected void mainPro() {
		...
		String regex = "(?s)(?i)(self.location=\"/)(News/ShowClass.asp\\?ClassID=[0-9]+&\\w+=\\w+)(\";)";
		String regex1 = "(?s)(?i)(self.location=\"/)(\\?\\w+=\\w+)(\";)";
		String location = "";
		String[] results = JavaUtil.match(webContent, regex);
		if ((results != null) && (results.length > 2)) {
			location = results[2];
			location = "&" + location;
			webContent = getPageContent(location);
		}
		String[] results1 = JavaUtil.match(webContent, regex1);
	...
	protected String getPageContent(String location) {
		...
		String webContent = "";
		String downloadUrl = link.getUrl();
		downloadUrl = downloadUrl + location;
		NanyangWebPageDownloader downloader = new NanyangWebPageDownloader(downloadUrl);
		downloader.setCheckCookie(true);
		downloader.setCookieStr(task.getUcookies());
		downloader.setCheckRedirect(true);
...
//		WebShieldSessionVerify=NOIYdUvxjRRIVW38hwA9
		String webContent = "self.location='/News/ShowClass.asp?ClassID=52&WebShieldSessionVerify=KLPunvShGw4qD9pcWFkG';}</script><script>setTimeout('JumpSelf()',700);</script></head><body></body></html>";
		String regex = "(?s)(?i)(self.location='/)(News/ShowClass.asp\\?ClassID=[0-9]+&\\w+=\\w+')(;)";
		String location = "";
		String[] results = JavaUtil.match(webContent, regex);
		if ((results != null) && (results.length > 2)) {
