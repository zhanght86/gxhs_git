...
public class PageResolver_Nanyang extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Nanyang.class);

	public PageResolver_Nanyang(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		String regex = "[\\u4E00-\\u9FA5]";
		String[] chinese = JavaUtil.match(ne.getNews_content(), regex);
		if (null == chinese || chinese.length < 0 || StringUtils.isEmpty(ne.getNews_title())) {
			ne = null;
			return ne;
		}
		if(ne.getNews_title().contains("无标题") || ne.getNews_content().length() < 50) {
			ne = null;
		}
		return ne;
