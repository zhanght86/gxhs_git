...
public class PageCrawler_MoeGOV extends PageCrawler{

	public PageCrawler_MoeGOV(TaskLink link, Task task) {
		super(link, task);
	}
	/**
	...
	protected ArrayList<TaskLink> extractTaskLinks(String webContent, Task task, TaskLink link) {
		...
		for (int i = 0; i < urlList.size(); i++) {
			if(urlList.get(i).getUrl().contains("http://www.moe.gov.cn/s78/A03/ghs_left/moe_634/") || urlList.get(i).getUrl().contains("http://www.moe.gov.cn/s78/A03/moe_560/jytjsj_2014/2014_qg")) {
				urlList.remove(i);
			}
		}
		return urlList;
