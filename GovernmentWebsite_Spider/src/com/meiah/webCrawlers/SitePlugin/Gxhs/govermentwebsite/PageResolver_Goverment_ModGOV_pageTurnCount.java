 ...
 * @date 2015-10-9
 */
public class PageResolver_Goverment_ModGOV_pageTurnCount extends
		PageResolver_General {

	public PageResolver_Goverment_ModGOV_pageTurnCount(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		//获取正文翻页的链接（即2-最后 一页）,此网站的正文翻页只有国防教育这个栏目，其中主要是图片进行翻页，此时只保存翻页的图片，内容都是一样的
		...
		ne.setNews_title(StringUtils.replaceEach(newsTitle, new String[] { "组图"}, new String[] { ""}));
		...
		if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_content().contains("您访问的页面不存在") || StringUtils.isEmpty(ne.getNews_title())) {
			ne = null;
		}
