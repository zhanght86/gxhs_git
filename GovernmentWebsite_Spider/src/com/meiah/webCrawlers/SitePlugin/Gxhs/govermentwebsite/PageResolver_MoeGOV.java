 ...
 * @date 2015-10-4
 */
public class PageResolver_MoeGOV extends PageResolver_General{

	public PageResolver_MoeGOV(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(documentNo.startsWith("第")) {
			documentNo = ne.getNews_title();
		}
		if(StringUtils.isNotBlank(documentNo))
			ne.setNews_notice_code(documentNo);
		link.setLinkType(SysConstants.PAGETYPE_CONTENT);
		
		if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_content().length() < 20 || StringUtils.isEmpty(ne.getNews_title())) {
			ne = null;
		}
