...
public class PageResolver_Lntb extends PageResolver_General {
	
	public PageResolver_Lntb(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(content.contains("点击浏览该文件")){
			content = content.replaceAll("点击浏览该文件","");
		}
		...
		for(String s:newsImage){
		      if(s.equals("http://www.lntb.gov.cn/images/doc.gif")){
		    	   newsImage.remove(s); 
		       }
		  }
