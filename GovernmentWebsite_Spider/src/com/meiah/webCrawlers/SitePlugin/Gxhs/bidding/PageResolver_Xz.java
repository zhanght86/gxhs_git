...
public class PageResolver_Xz extends PageResolver_General {
	
	public PageResolver_Xz(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(content.contains("ajax_usercount")){
			content = null;
		}
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://www.nmgp.gov.cn/procurement/jc_images/download.png")){
		        	 it.remove();
		         }
		     }
			if(!newsImage.isEmpty())
		...
		if(StringUtils.isNotBlank(columnName)){
			if(columnName.equals("招标更正公告")||(columnName.equals("重要通知"))){
				ne = null;
				return ne;
				}
