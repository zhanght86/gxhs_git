...
public class PageResolver_Qhwszwdt extends PageResolver_General {
	
	public PageResolver_Qhwszwdt(Task task) {
		super(task);
	}
...
//		}
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.equals("http://qhzbtb.qhwszwdt.gov.cn/qhzbtb/eWebEditor/sysimage/icon16/doc.gif")){
		        	 it.remove();
			       }
			     if(s.equals("http://qhzbtb.qhwszwdt.gov.cn/qhzbtb/eWebEditor/sysimage/icon16/pdf.gif")){
			    	  it.remove();
			       }
		     }
			if(!newsImage.isEmpty())
