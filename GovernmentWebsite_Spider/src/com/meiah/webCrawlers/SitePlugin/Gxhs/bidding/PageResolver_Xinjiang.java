...
public class PageResolver_Xinjiang extends PageResolver_General {
	
	public PageResolver_Xinjiang(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://mail.qq.com/")){
		        	 it.remove();
		         }
		     }
			if(!newsImage.isEmpty())
		...
		if(columnName.equals("协议供货及定点采购")||(columnName.equals("供应商频道"))||(columnName.equals("常见问题"))
				||(columnName.equals("网上登记"))){
			columnName= null;
			ne = null;
			return ne;
			}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter  || ne.getNews_title().length() < 5) {
				ne = null;
				return ne;
			}else if(ne.getNews_content().isEmpty()){
				if(newsImage.isEmpty()){
					if(accessList.isEmpty()){
						ne = null;
						return ne;
