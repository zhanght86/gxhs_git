...
public class PageResolver_Amoney extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Amoney.class);
	
	public PageResolver_Amoney(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			ne.setPage_source(newsSource.replace(" ", "").replace("\r\n", ""));
		}
	...
	protected String getNewsSource(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (newsSource == null || newsSource.equals("")) {
				String regex = "(?s)(?i)(来源\\s+|來源：|来源：|來源:|来源:|来自:)([\\u4E00-\\u9FA5]+[0-9]+|[\\u4E00-\\u9FA5]+-[\\u4E00-\\u9FA5]+|\\s*[\\u4E00-\\u9FA5]+)";
