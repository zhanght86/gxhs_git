...
public class PageResolver_EastDay extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_EastDay.class);
	
	public PageResolver_EastDay(Task task) {
		super(task);
	}
...
//		String prefixUrl = link.getRefererUrl().substring(0,link.getRefererUrl().indexOf("://") + 3);
		ne.setWebsite_url("http://news.eastday.com/");
...
//			ne.setIsHomePageNews(1);
		...
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;" , ">>>" , "&amp;nbsp" , " " }, new String[] { "", "" ,"" , "" , "" , "" , "" , ""});
		...
		if(pageUrl.contains("http://imedia.eastday.com/")) {
			ne = null;
		}
		return ne;
	}
