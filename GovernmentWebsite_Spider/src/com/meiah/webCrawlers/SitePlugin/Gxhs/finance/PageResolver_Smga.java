...
public class PageResolver_Smga extends PageResolver_General {
	
	public PageResolver_Smga(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(content.contains("下一页")||content.contains("上一页")){
			content = null;
		}
		if(!(content==null)){
		...
		try {
			ne.setServer_ip(InetAddress.getLocalHost().getHostAddress());
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
		if (logger.isDebugEnabled()) {
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("：", "").replace("未知", ""));
		...
		if(StringUtils.isNotBlank(columnName)){
			if(columnName.equals("会员专区")||columnName.equals("行业标准")||columnName.equals("实用信息")||columnName.equals("燃气器具")){
				ne =null;
				return ne;
			}
