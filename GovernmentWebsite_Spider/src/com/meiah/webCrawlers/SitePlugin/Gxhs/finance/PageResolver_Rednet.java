...
public class PageResolver_Rednet extends PageResolver_General {
	
	public PageResolver_Rednet(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(StringUtils.replaceEach(newsSource, new String[]{"来源：","稿源：","[","]"}, new String[]{"","","",""}));
		String newsAuthor = getNewsAuthor(task.getSiteConfig(), page, doc);
		if(StringUtils.isNotBlank(newsAuthor))
			ne.setNews_author(StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?" ,"[","]","记者","通讯员"}, new String[] { "", "" ,"" , "","", "" ,"" , "","",""}));
