...
public class PageResolver_Fjsen extends PageResolver_General {
	public PageResolver_Fjsen(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_content())) {
			if(ne.getNews_content().length() < 100 || content.startsWith("copyright")) {
				ne = null;
			}
		}
		return ne;
