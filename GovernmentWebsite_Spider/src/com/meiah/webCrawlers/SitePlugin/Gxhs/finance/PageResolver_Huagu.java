...
public class PageResolver_Huagu extends PageResolver_General {
	
	public PageResolver_Huagu(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("：")){
				newsSource = newsSource.replace(" ", "").replace("来源", "").replace("：", "");
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://imgcache.huagu.com/img/article/img_huaxun.gif")){
		        	 it.remove();
		         }	
		     }
			if(!newsImage.isEmpty())
