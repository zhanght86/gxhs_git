...
public class PageResolver_Hexunguba extends PageResolver_General {
	
	public PageResolver_Hexunguba(Task task) {
		super(task);
	}
	...
	protected String getNewsColumnName(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				...
				String regex = "(?s)(?i)(股票|首页)(\\s*»\\s*|\\s*>>\\s*|\\s*>\\s*|\\s*->\\s*|\\s*-\\s*|\\s*/\\s*|\\s*—\\s*|\\s*→\\s*"
