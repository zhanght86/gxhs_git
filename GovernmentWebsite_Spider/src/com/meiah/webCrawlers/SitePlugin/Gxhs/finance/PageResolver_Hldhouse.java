...
public class PageResolver_Hldhouse extends PageResolver_General {
	
	public PageResolver_Hldhouse(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源：")){
				newsSource = newsSource.substring(newsSource.indexOf("来源："));
				ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("未知", "").replace("/", "").replace("：", "").replace("次-->", ""));
			}
		}
