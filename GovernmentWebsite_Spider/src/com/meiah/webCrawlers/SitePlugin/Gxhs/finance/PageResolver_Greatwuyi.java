...
public class PageResolver_Greatwuyi extends PageResolver_General {

	public PageResolver_Greatwuyi(Task task) {
		super(task);
	}
...
//		ne.setSpot_code(Config.getSpotID());
		...
		ne.setWebsite_name(siteName.replaceAll("\r\n", ""));
...
//			ne.setIsHomePageNews(1);
		...
		ne.setNews_content(content.replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", "")
				.replaceAll("设为首页", "").replaceAll("广告合作", "").replaceAll("即刻搜索", "").trim());
		...
		if(StringUtils.isNotBlank(newsSource)){
				newsSource = newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "");
			if(newsSource.contains("作者::")) {
				newsSource = newsSource.substring(newsSource.indexOf("作者::") + 1 );
			}
			newsSource = newsSource.replace("者::", "");
