...
public class PageResolver_Aviationnow extends PageResolver_General {
	
	public PageResolver_Aviationnow(Task task) {
		super(task);
	}
...
//		}
		if(content.contains("webmaster@aviationnow.com.cn")){
			content = null;
		}
...
//		}
		...
		newsAuthor = StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?" }, new String[] { "", "" ,"" , "","",""});
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				...
				if ((temp != null) && (temp.length >= 3))
					columnName = temp[2].replace("首页 >", "").replace(">","");
