...
public class PageResolver_Hf365 extends PageResolver_General {
	
	public PageResolver_Hf365(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("编辑") ) {
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("编辑") + 3 );
			} 
		...
		if(StringUtils.isNotBlank(columnName))
			ne.setNews_column(columnName.replaceAll("正文", ""));
	...
	protected String getNewsColumnName(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				...
				String regex = "(?s)(?i)(首页|位置：|位置:|位置 ：新闻)(\\s*»\\s*|\\s*>>\\s*|\\s*>\\s*|\\s*->\\s*|\\s*-\\s*|\\s*\\|\\s*|\\s*/\\s*|\\s*—\\s*|\\s*→\\s*"
