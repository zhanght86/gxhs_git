...
public class PageResolver_EastMoneyRenwu extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_EastMoneyRenwu.class);
	
	public PageResolver_EastMoneyRenwu(Task task) {
		super(task);
	}
...
//		String prefixUrl = link.getRefererUrl().substring(0,link.getRefererUrl().indexOf("://") + 3);
		ne.setWebsite_url("http://www.eastmoney.com/");
...
//			ne.setIsHomePageNews(1);
		...
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;" , "点击查看>>>" }, new String[] { "", "" ,"" , "" , "" , ""});
		...
		if(StringUtils.isNotBlank(newsSource)){
			ne.setPage_source(newsSource.replace(" ", ""));
		} else {
			ne.setPage_source("东方财富网");
		}
		String newsAuthor = getNewsAuthor(task.getSiteConfig(), page, doc);
		if(StringUtils.isNotBlank(newsAuthor)){
			ne.setNews_author(StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?" , "编辑：" }, new String[] { "", "" ,"" , "" , "" , "" , ""}));
		} else if (StringUtils.isEmpty(newsAuthor) || newsAuthor.length() <= 1) {
			if(StringUtils.isNotBlank(ne.getNews_title())) {
				String titleName = ne.getNews_title();
				if(titleName.contains("：")) {
					String regex = "(?s)(?i)([\\u4E00-\\u9FA5]{2,4})(：)";
					String[] name = JavaUtil.match(titleName, regex);
		...
		if(StringUtils.isNotBlank(ne.getNews_content())) {
			if(ne.getNews_content().length() < 100 || ne.getNews_content().contains("�")) {
				ne = null;
				return ne;
			}
		} 
		if(ne.getNews_title().endsWith("网点信息")) {
			ne = null;
		}
		return ne;
	}
	public static void main(String[] args) {
		String web = "陶冬：人民币需不需要大幅贬值取决于中国经济的基本面";
		String regex = "(?s)(?i)([\\u4E00-\\u9FA5]{2,4})(：)";
		String[] name = JavaUtil.match(web, regex);
		if(name.length > 2) {
			String nn = name[1];
		}
	}
