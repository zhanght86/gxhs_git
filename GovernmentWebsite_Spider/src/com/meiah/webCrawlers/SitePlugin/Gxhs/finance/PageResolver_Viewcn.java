...
public class PageResolver_Viewcn extends PageResolver_General {
	
	public PageResolver_Viewcn(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("台湾", ""));
