...
public class PageResolver_Hangzhou extends PageResolver_General {
	
	public PageResolver_Hangzhou(Task task) {
		super(task);
	}
	...
	protected String getNewsColumnName(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				...
				String regex = "(?s)(?i)(首页|位置：|位置:|位置)(\\s*»\\s*|\\s*>>\\s*|\\s*>\\s*|\\s*->\\s*|\\s*-\\s*|\\s*/\\s*|\\s*—\\s*|\\s*→\\s*"
		...
		try {
			...
			if (newsAuthor == null || newsAuthor.equals("")) {
				...
				if (newsAuthor.equals("")) {
					String regex = "(?s)(?i)(作者：记者|作者：通讯员|作者：见习记者|作者：实习记者|作者：实习生|作者：|作者:|作者\\s+|作者︰|作者 :|编辑：|編輯\\s+|編輯：|編輯:|编辑:|编辑\\s+|\\（记者：|\\（记者:|\\(记者：|\\(记者:|发布者：|发布者\\s+|发布者:|\\d+\\s+\\|\\s+)([\\u4E00-\\u9FA5]{1}\\s+[\\u4E00-\\u9FA5]{1}|\\s*[\\u4E00-\\u9FA5]+\\s*|\\s*\\[[\\u4E00-\\u9FA5]+\\])";
				...
				if(newsAuthor.contains("编辑")){
					newsAuthor = newsAuthor.replaceAll("编辑", "");
				}
