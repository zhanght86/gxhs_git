...
public class PageResolver_Details extends PageResolver_General {
	public PageResolver_Details(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("：", ""));
	...
	protected String getNewsColumnName(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				webContent = webContent.replaceAll("大太阳首页", "");
