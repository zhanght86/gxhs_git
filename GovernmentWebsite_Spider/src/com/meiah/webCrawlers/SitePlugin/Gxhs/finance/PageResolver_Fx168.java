...
public class PageResolver_Fx168 extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Fx168.class);
	
	public PageResolver_Fx168(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("文 / ")) {
				newsAuthor = newsAuthor.replaceAll("文 / ", "");
			} else {
				newsAuthor = "";
			}
