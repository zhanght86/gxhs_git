...
public class PageResolver_Dizhijixie extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Dizhijixie.class);
	
	public PageResolver_Dizhijixie(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			ne.setPage_source(newsSource.replaceAll("来源：", "").replaceAll(" ", "").replaceAll("\r\n", ""));
		}
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			...
			if(newsAuthor.contains("作者")) {
				...
				if(newsAuthor.contains("浏览次数")) {
					newsAuthor = newsAuthor.substring(0, newsAuthor.indexOf("浏览次数"));
				} else {
		...
		if(!pageUrl.matches(".*" + "/\\d{4}-\\d{2}/" + ".*.html")) {
			ne = null;
		}
		return ne;
	}
	public static void main(String[] args) {
		String pageUrl = "http://tele.ofweek.com/2015-09/ART-8320505-8120-29009364.htmllll";
		if(pageUrl.matches(".*" + "/\\d{4}-\\d{2}/" + ".*.html") ) {
		}
	}
}
