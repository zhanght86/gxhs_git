...
public class PageResolver_010lf extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_010lf.class);
	
	public PageResolver_010lf(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.length()>15){
				newsSource = newsSource.substring(15);
				if(newsSource.contains("我要")){
					newsSource = null; 
				}
			}else{
				newsSource = null; 
			...
			if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("：", "").replace("微博", "").replace("朱景锋", "").replace("作者", ""));
		}
