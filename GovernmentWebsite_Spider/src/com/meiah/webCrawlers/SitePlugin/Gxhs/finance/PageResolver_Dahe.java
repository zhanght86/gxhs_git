...
public class PageResolver_Dahe extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_General.class);
	
	public PageResolver_Dahe(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		ne.setNews_content(
				content.replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", "").replaceAll("&ldquo", "")
						.replaceAll("&rdquo", "").replaceAll("&hellip", "").replaceAll("&middot", "")
						.replaceAll("&mdash", "").replaceAll("移动用户发送HNZB到10658000，订阅河南手机报。早报+晚报，每天一角钱。", "")
						.replaceAll("移动用户发送hnzb到10658000，订阅河南手机报。早报+晚报，每天一角钱。", "").trim());
		...
		if (StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源") ) {
				newsSource = newsSource.substring(newsSource.indexOf("：") + 1 );
			} 
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || titles.length() < 5 
					|| titles.endsWith("下载") || titles.endsWith("解答") || titles.endsWith("须知")) {
				ne = null;
				return ne;
			}
		}
