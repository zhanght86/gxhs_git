...
public class PageResolver_Ccpdd extends PageResolver_General {
	
	public PageResolver_Ccpdd(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		String rep = "";
		if(content.contains("600)makesmallpic(this,600,1800)")) {
			rep = content.substring(content.indexOf("600)makesmallpic(this,600,1800)"));
			rep = rep.substring(0, rep.indexOf("/>") + 2);
			content = StringUtils.replaceEach(content, new String[] {rep}, new String[] { ""});
		}
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;",rep }, new String[] { "", "" ,"" , "","",""});
