 ...
 *
 */
public class PageResolver_Modernbankers extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_General.class);
	
	public PageResolver_Modernbankers(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("作者：")&&newsAuthor.contains("时间")){
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者：")+1, newsAuthor.indexOf("时间"));
			}
			ne.setNews_author(StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?" ,"者："}, new String[] {"", "", "" ,"" , "","",""}));
			}
