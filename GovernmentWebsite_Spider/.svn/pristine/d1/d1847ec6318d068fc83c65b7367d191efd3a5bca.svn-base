 ...
 * @date 2015-10-19
 */
public class PageResolver_Caep_pdf extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Caep_pdf.class);
	public PageResolver_Caep_pdf(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 
					|| ne.getNews_content().contains("点击查看")) {
				ne = null;
				return ne;
			}
		}
