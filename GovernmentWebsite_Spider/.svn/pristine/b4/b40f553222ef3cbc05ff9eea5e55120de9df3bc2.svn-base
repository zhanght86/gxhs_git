 ...
 * @date 2015-10-7
 */
public class PageResolver_Xjzfcs extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Xjzfcs.class);
	public PageResolver_Xjzfcs(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5
					|| ne.getNews_title().contains("•") || ne.getNews_title().endsWith("作品")) {
				ne = null;
				return ne;
			}
		}
