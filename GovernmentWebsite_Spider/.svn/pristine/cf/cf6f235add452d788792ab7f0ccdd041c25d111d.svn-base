 ...
 *
 */
public class PageResolver_Fjzc extends PageResolver_General {
	
	public PageResolver_Fjzc(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源：")&&newsSource.contains("进入论坛")){
				newsSource = newsSource.substring(newsSource.indexOf("来源：")+3, newsSource.indexOf("进入论坛"));
			}else if(newsSource.contains("来源：")){
				newsSource = newsSource.substring(newsSource.indexOf("来源：")+3);
			}else if(newsSource.contains("来源：")&&newsSource.contains("&gt")){
				newsSource = newsSource.substring(newsSource.indexOf("来源：")+3,newsSource.indexOf("&gt"));
			}
