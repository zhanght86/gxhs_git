...
public class PageResolver_Ofweek extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Ofweek.class);
	
	public PageResolver_Ofweek(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 
					|| ne.getNews_title().endsWith("近期活动")) {
				ne = null;
				return ne;
			}
		}
