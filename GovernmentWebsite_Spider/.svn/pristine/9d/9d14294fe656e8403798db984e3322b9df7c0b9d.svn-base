...
public class PageCrawler_MostGOV extends PageCrawler{

	public PageCrawler_MostGOV(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected ArrayList<TaskLink> extractTaskLinks(String webContent, Task task, TaskLink link) {
		...
		for (int i = 0; i < urlList.size(); i++) {
			if(urlList.get(i).getUrl().contains("http://www.most.gov.cn/eng")) {
				urlList.remove(i);
			}
//			System.out.println(urlList.get(i).getUrl());
	...
	protected void mainPro() {
		...
		if (webContent.length() < 100 && !link.getUrl().endsWith(".pdf")) {
			logger.warn("网页内容过短？！:" + link.getUrl());
			return;
		}
		long t = System.currentTimeMillis();
		// if (task.getMaxLevel() == 1) {
		// addLinkOneLevel(webContent);
		// } else {
		addLink(webContent);
		// }
		if (logger.isDebugEnabled())
