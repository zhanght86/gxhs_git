 ...
 * @date 2015-10-7
 */
public class PageResolver_Dgskdg extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Dgskdg.class);
	public PageResolver_Dgskdg(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		ne.setNews_content(content.replaceAll("、中、", "").replace("、中、小", "").replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", ""));
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(ne.getNews_title().contains("决算") || ne.getNews_title().contains("预算") || StringUtils.isEmpty(ne.getNews_content()) 
					|| ne.getNews_title().length() <= 4) {
				ne = null;
				return ne;
			}
		}
