 ...
 * @date 2015-10-7
 */
public class PageResolver_Ie_Cass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Ie_Cass.class);
	public PageResolver_Ie_Cass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(content.contains("经济所作者：")) {
			newsAuthor = content.substring(content.indexOf("：") + 1 , content.indexOf("责任者"));
			String replace = content.substring(content.indexOf("经济所作者："),content.indexOf("　"));
