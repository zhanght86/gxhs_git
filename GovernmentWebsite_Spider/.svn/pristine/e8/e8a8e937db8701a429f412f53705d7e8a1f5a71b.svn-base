...
public class PageResolver_Hnbynews extends PageResolver_General {
	
	public PageResolver_Hnbynews(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		String replace = "";
		if(content.contains("Arial")) {
			replace = content.substring(content.indexOf("Arial"));
			replace = replace.substring(0, replace.indexOf(">") + 1);
		}
		String replace1 = "";
		if(content.contains("color")) {
			replace1 = content.substring(content.indexOf("color"));
			replace1 = replace.substring(0, replace.indexOf(">") + 1);
		}
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>","(?s)(?i)<.*?/>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;",replace,replace1 }, new String[] { "" ,"" , "", "" ,"" , "","",""});
		...
		if(StringUtils.isNotBlank(newsSource.replace("淮南网", "").replace("记着", "")))
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://www.huainannet.com/images/weixin-big.jpg")){
		        	 it.remove();
		         }	
		     }
			if(!newsImage.isEmpty())
