 ...
 * @date 2015-10-7
 */
public class PageResolver_Hlass extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Hlass.class);
	public PageResolver_Hlass(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().contains("名单") || ne.getNews_title().contains("视频")|| ne.getNews_content().contains("新版本的adobeflashplayer")					|| ne.getNews_title().length()<=3) {
				ne = null;
				return ne;
			}
		}
