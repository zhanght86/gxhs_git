...
public class PageResolver_Bidhome extends PageResolver_General {
	
	public PageResolver_Bidhome(Task task) {
		super(task);
	}
	...
	protected NewsGeneric resloveNewsPage(WebPage page) {
		...
		String page_url=link.getUrl();
		if(page_url.contains("</tbody>")||page_url.contains("</a>")||page_url.contains("<br/>")
				||page_url.contains("</div>")){
			page_url =null; 
		}
		ne.setPage_url(page_url);
...
//			ne.setIsHomePageNews(1);
		...
		content=(content.replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", "").replaceAll("业主资料请 登录 后查看,若您还未注册,请先 免费注册", "")
				.replaceAll("注：投标人采用串标、围标、挂靠方式，或以行贿及其他手段弄虚作假谋取中标的， 投标及中标无效。招标人可立即没收其投标保证金，给招标人造成的损失超过投标保证 金数额的，还应当对超过部分予以赔偿。", "").trim());
		if(content.contains("公告内容:")){
			String[]contents = content.split("公告内容:");
				String contentt = contents[1];
				if(contentt.trim().isEmpty()){
					content = null;
				}else if(contentt.contains("项目信息")){
					content = null;
				}
			}else if(content.contains("中标内容:")){
			String[]contents = content.split("中标内容:");
				String contentt = contents[1];
				if(contentt.trim().isEmpty()){
					content = null;
				}
