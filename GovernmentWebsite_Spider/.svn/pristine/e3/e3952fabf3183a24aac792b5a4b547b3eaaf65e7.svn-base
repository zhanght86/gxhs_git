...
public class PageCrawler_Cicir extends PageCrawler {
	
	public static final String TODAY = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	public PageCrawler_Cicir(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected ArrayList<TaskLink> extractTaskLinks(String webContent, Task task, TaskLink link) {
		...
		String[] fileExtetions = ".xls,.xml,.txt,.jpg,.mp3,.mp4,.doc,.mpg,.mpeg,.jpeg,.gif,.png,.js,.zip,.rar,.exe,.swf,.rm,.ra,.asf,.css,.bmp,.z,.gz,.tar,.cpio,.class".split(",");
	...
	protected String getPageContent() {
		...
		if(downloadUrl.endsWith(".pdf")) {
			String lastPDFUrl = downloadUrl;
			String str = lastPDFUrl.substring(lastPDFUrl.lastIndexOf("/") + 1 , lastPDFUrl.lastIndexOf(".pdf"));
			str = URLEncoder.encode(str);
			String prefix = lastPDFUrl.substring(0,lastPDFUrl.lastIndexOf("/") + 1);
			lastPDFUrl = prefix + str + ".pdf";
			String content = PDFPaserUtil.pasePDF2TextByHttp(lastPDFUrl);
			webContent = content.replaceAll(" ", "").replaceAll("\r\n", "    ");
		} else {
