...
public class PageResolver_Huanbohainews extends PageResolver_General {
	
	public PageResolver_Huanbohainews(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>","(?s)(?i)<.*?/>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;","&mdash;"}, new String[] {"","", "" ,"" , "","",""});
		...
		if(StringUtils.isNotBlank(newsSource)){
			...
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("未知", "").replace("/", "").replace("：", "").replace("莲花财经", "").replace("nbsp;", ""));
		}
