 ...
 * @date 2015-10-19
 */
public class PageResolver_Cicir extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Cicir.class);

	public PageResolver_Cicir(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() <= 4
					|| ne.getNews_content().contains("资料正在整理中。。。")
					|| StringUtils.isEmpty(ne.getNews_content())
					|| ne.getNews_content().contains("暂无相关新闻!")
					|| ne.getNews_content().contains(".swf")) {
				ne = null;
				return ne;
			}
		}
