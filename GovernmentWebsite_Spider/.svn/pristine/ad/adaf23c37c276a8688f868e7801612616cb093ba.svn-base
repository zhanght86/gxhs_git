...
public class PageResolver_Ftchinese extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Ftchinese.class);
	
	public PageResolver_Ftchinese(Task task) {
		super(task);
	}
	...
	protected NewsGeneric resloveNewsPage(WebPage page) {
		...
		if(webContent.contains("全文") || webContent.contains("下一页")) {
			WebPageDownloader wd = new WebPageDownloader(pageUrl + "?full=y");
			try {
				webContent = wd.getPageContent();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
