...
public class PageResolver_Zdy333 extends PageResolver_General {
	
	public PageResolver_Zdy333(Task task) {
		super(task);
	}
...
//		String prefixUrl = link.getRefererUrl().substring(0,link.getRefererUrl().indexOf("://") + 3);
		ne.setWebsite_url("http://www.zdy333.com/");
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replaceAll("\r\n", "").replaceAll("\t", "").replaceAll(":", "").replace("来源：", "").trim());
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || ne.getNews_title().length() < 5 || newsTitle.endsWith("通知") || newsTitle.endsWith("安排")) {
				ne = null;
				return ne;
			}
		}
