...
public class PageResolver_Hz66 extends PageResolver_General {
	
	public PageResolver_Hz66(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			newsAuthor = StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?" ,"\r\n","作者","来源："}, new String[] {"",	 "", "" ,"" , "","","","",""});
			if(StringUtils.isNotBlank(newsAuthor))
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://mmbiz.qpic.cn/mmbiz/")){
		        	 it.remove();
		         }	
		     }
			if(!newsImage.isEmpty())
