...
public class PageResolver_Ihuacai extends PageResolver_General {
	
	public PageResolver_Ihuacai(Task task) {
		super(task);
	}
		...
		try {
			...
			if (newsAuthor == null || newsAuthor.equals("")) {
				...
				if (newsAuthor.equals("")) {
					...
					if(newsAuthor.equals("")) {
						regex = "(?s)(?i)(作者：|作者:|作者\\s+|作者︰|作者 :|編輯\\s+|編輯：|編輯:|编辑：|编辑:|\\（记者：|\\（记者:|\\(记者：|\\(记者:|发布者：|发布者\\s+|发布者:|\\d+\\s+\\|\\s+)([\\u4E00-\\u9FA5]+\\s+[\\u4E00-\\u9FA5]+{2,4})";
						results = JavaUtil.match(webContent, regex);
						if ((results != null) && (results.length > 2))
							newsAuthor = results[2];
					}
	...
	protected String getNewsSource(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (newsSource == null || newsSource.equals("")) {
				String regex = "(?s)(?i)(來源：|来源：|來源:|来源:|来自:)([\\u4E00-\\u9FA5]+[0-9]+|[\\u4E00-\\u9FA5]+-[\\u4E00-\\u9FA5]+|[\\u4E00-\\u9FA5]+)";
				webContent = webContent
						.replaceAll("(?i)(?s)<marquee.*?</marquee>", "")
						.replaceAll("(?s)(?i)<.*?>", " ")
						.replaceAll("\\s+", " ");
