...
public class PageResolver_Dop2p extends PageResolver_General {
	
	public PageResolver_Dop2p(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>","(?s)(?i)<.*?/>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;","(?s)(?i)<.*?/>" }, new String[] { "" , "", "" ,"" , "","",""});
