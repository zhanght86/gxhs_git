...
public class PageResolver_Hexun extends PageResolver_General {
	
	public PageResolver_Hexun(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource))
			ne.setPage_source(newsSource.replaceAll("来源：", ""));
	...
	protected String getNewsColumnName(SiteConfig conf, WebPage page, Document doc) {
		...
		try {
			...
			if (StringUtils.isEmpty(columnName)) {
				webContent = webContent.replaceAll("手机和讯", "").replaceAll("新闻频道", "");
				String regex = "(?s)(?i)(和讯网|首页)(\\s*»\\s*|\\s*>>\\s*|\\s*>\\s*|\\s*->\\s*|\\s*-\\s*|\\s*\\|\\s*|\\s*/\\s*|\\s*—\\s*|\\s*→\\s*"
