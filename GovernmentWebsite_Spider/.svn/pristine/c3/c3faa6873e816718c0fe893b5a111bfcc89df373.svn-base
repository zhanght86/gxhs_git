...
public class PageResolver_BmpSoft extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_BmpSoft.class);
	
	public PageResolver_BmpSoft(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源:")) {
				newsSource = newsSource.substring(newsSource.indexOf("来源:") + 3);
				if(newsSource.contains("作者")) {
					newsSource = newsSource.substring(0,newsSource.indexOf("作者"));
					newsSource.replaceAll(" ", "");
				} else {
					newsSource = newsSource.substring(0,newsSource.indexOf(" "));
				}
			} else {
				newsSource = "";
			}
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("作者:")) {
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者:") + 3);
				if(newsAuthor.contains("点击")) {
					newsAuthor = newsAuthor.substring(0,newsAuthor.indexOf("点击"));
					newsAuthor.replaceAll(" ", "");
				} else if(StringUtils.isNotBlank(newsAuthor)){
					newsAuthor = newsAuthor.substring(0,newsAuthor.indexOf(" "));
				}
			} else {
				newsAuthor = "";
			}
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 || ne.getNews_title().endsWith("近期活动")) {
				ne = null;
				return ne;
			}
		}
