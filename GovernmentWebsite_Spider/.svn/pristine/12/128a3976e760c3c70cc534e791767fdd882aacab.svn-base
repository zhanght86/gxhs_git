 ...
 * @date 2015-10-7
 */
public class PageResolver_Zjss extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Zjss.class);
	public PageResolver_Zjss(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5 
					|| ne.getNews_title().contains("通知") || ne.getNews_title().contains("公告") || ne.getNews_title().contains("公示")) {
				ne = null;
				return ne;
			}
		}
