...
public class PageResolver_Ztbw extends PageResolver_General {
	
	public PageResolver_Ztbw(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(content.contains("。2、请检查输入的网址是否正确。3、如果不能确认输入的网址，请浏览网站首页来查看所要访问的网址。")||content.contains("您还没有登录，请登录后查看联系方式免费注册为会员后") ){
			content = null;
		}
...
//		}
		ne.setNews_content(content.replaceAll("　", "").replaceAll("\r\n", "").replaceAll("\n", "").replaceAll("为保证您能够顺利投标，请在投标或购买招标文件前向招标代理机构或招标人咨询投标详细要求，具体要求及项目情况以招标代理机构或招标人的解释为准。","").trim());
