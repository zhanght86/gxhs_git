...
public class PageResolver_Hysec extends PageResolver_General {
	
	public PageResolver_Hysec(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源：")&&newsSource.contains("时间：")){
				newsSource = newsSource.substring(newsSource.indexOf("来源:")+4, newsSource.indexOf("时间："));
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("作者：")){
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者：")+3).trim();
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://www.hysec.com/hyzq/hy/detail/images/ico_xz.gif")){
		        	 it.remove();
		         }	
		     }
			if(!newsImage.isEmpty())
