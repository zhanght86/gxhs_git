...
public class PageResolver_Gemag extends PageResolver_General {
	
	public PageResolver_Gemag(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源：")&&newsSource.contains("作者：")){
				newsSource = newsSource.substring(newsSource.indexOf("来源：")+3, newsSource.indexOf("作者："));
				ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("未知", "").replace("/", "").replace("|", ""));
			}
		}
		String newsAuthor = getNewsAuthor(task.getSiteConfig(), page, doc);
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("作者：")&&newsAuthor.contains("阅读：")){
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者：")+3, newsAuthor.indexOf("阅读：")).trim();
				ne.setNews_author(StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","?","编辑","客服","/","|","本文来源：21世纪经济报道"}, new String[] {"","","","","","", "" ,"" , "","",""}).trim());
			}
		}
