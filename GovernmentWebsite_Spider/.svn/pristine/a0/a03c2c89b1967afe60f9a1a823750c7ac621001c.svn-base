...
public class PageCrawler_Huanqiu extends PageCrawler {
	
	public static final String TODAY = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	public PageCrawler_Huanqiu(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected void addLink(String webContent) {
		if (this.link.getLevel() < this.task.getMaxLevel()) {
			...
			for (int i = 0; i < urlList.size(); i++) {
				...
				if(urlTobeAdd.contains("http://finance.huanqiu.com/financepic/")||
				   urlTobeAdd.contains("http://finance.huanqiu.com/pictures/")||
				   urlTobeAdd.contains("http://finance.huanqiu.com/br/photo/")||
				   urlTobeAdd.contains("http://finance.huanqiu.com/expo/tssb/")
				   ) {
					continue;
				}
