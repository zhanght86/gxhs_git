...
public class PageResolver_Shanxi extends PageResolver_General {
	
	public PageResolver_Shanxi(Task task) {
		super(task);
	}
...
//		}
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("C:UserslenovoAppDataLocalTempksohtmlwps6F69.tmp.png")){
		        	 it.remove();
		         }
		     }
			if(!newsImage.isEmpty())
