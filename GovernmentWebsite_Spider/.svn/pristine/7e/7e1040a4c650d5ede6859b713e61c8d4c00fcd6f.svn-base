...
public class PageResolver_Dfdaily extends PageResolver_General {
	public PageResolver_Dfdaily(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			...
			if(titleFilter || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 8 || ne.getNews_title().endsWith("近期活动")) {
				ne = null;
				return ne;
			}
		}
