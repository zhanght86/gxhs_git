...
public class PageResolver_FwXinhua extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_FwXinhua.class);
	
	public PageResolver_FwXinhua(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("／文")) {
				ne.setNews_author(StringUtils.replaceEach(newsAuthor, new String[] { "等", "撰写","作者：","作者:"," ","本刊记者" , "编辑：" , "／文" }, new String[] { "", "" ,"" , "" , "" , "" , "" , ""}));
			}
		}
