...
public class PageCrawler_Lass extends PageCrawler {
	
	public static final String TODAY = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	public PageCrawler_Lass(TaskLink link, Task task) {
		super(link, task);
	}
	...
	protected String getPageContent() {
		...
		if(webContent.contains("class='mainContent'") || webContent.contains("class='mainContent mainContentList'")) {
			StringBuffer webCon = new StringBuffer();
			String right = "";
			right = webContent.substring(webContent.lastIndexOf("网站首页") , webContent.lastIndexOf("id='footer'"));
			webCon = webCon.append(right);
			webContent = webCon.toString();
		}
		if(webContent.contains("class='mainContent Contentbox'")) {
			StringBuffer webCon = new StringBuffer();
			String main = "";
			main = webContent.substring(webContent.lastIndexOf("网站首页") , webContent.lastIndexOf("id='footer'"));
