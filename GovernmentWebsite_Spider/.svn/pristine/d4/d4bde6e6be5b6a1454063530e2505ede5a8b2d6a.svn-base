...
public class PageResolver_Hx168 extends PageResolver_General {
	
	public PageResolver_Hx168(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		content = StringUtils.replaceEach(content, new String[] { "(?s)(?i)<.*?>","(?s)(?i)<.*?/>", "&ldquo;","&lsquo;","&rsquo;","&rdquo;","全文：" }, new String[] { "","", "" ,"" , "","",""});
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源：")&&newsSource.contains("发布")){
				newsSource = newsSource.substring(newsSource.indexOf("来源：")+3, newsSource.indexOf("发布"));
			}
			ne.setPage_source(newsSource.replace(" ", "").replace("来源", "").replace("来源：", "").replace("来源:", "").replace("未知", "").replace("/", ""));
		}
		String newsAuthor = getNewsAuthor(task.getSiteConfig(), page, doc);
		if(StringUtils.isNotBlank(newsAuthor)){
			if(newsAuthor.contains("作者：")&&newsAuthor.contains("来源：")){
				newsAuthor = newsAuthor.substring(newsAuthor.indexOf("作者：")+3, newsAuthor.indexOf("来源：")).trim();
			}
		...
		if(!newsImage.isEmpty()){
			for(Iterator<String> it = newsImage.iterator();it.hasNext();){
		         String s = it.next();
		         if(s.contains("http://www.hx168.com.cn/hxzq/images/popo.jpg")){
		        	 it.remove();
		         }	
		     }
			if(!newsImage.isEmpty())
