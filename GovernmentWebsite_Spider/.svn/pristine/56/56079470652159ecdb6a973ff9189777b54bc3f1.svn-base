 ...
 * @date 2015-10-19
 */
public class PageResolver_Sociology2010 extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_Sociology2010.class);
	public PageResolver_Sociology2010(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		if(StringUtils.isNotBlank(ne.getNews_title())) {
			if(ne.getNews_title().contains("通知") || ne.getNews_title().contains("结果") || ne.getNews_title().contains("名单")
					|| ne.getNews_title().contains("招聘") || StringUtils.isEmpty(ne.getNews_content()) || ne.getNews_title().length() < 5) {
				ne = null;
				return ne;
			}
		}
