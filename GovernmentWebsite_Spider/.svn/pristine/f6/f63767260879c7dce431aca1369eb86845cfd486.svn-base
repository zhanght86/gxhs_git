...
public class PageResolver_FdBjx extends PageResolver_General {
	protected Logger logger = Logger.getLogger(PageResolver_FdBjx.class);
	
	public PageResolver_FdBjx(Task task) {
		super(task);
	}
...
//			ne.setIsHomePageNews(1);
		...
		content = content.replaceAll("德清新闻网版权声明：凡本网的所有新闻作品，版权均属于德清新闻网，未经本网授权不得转载、摘编或利用微博、微信、论坛和手机客户端等其它方式使用上述新闻作品。已经本网授权使用作品的，应在授权范围内使用，并注明“来源：德清新闻网”，以及该新闻作品的作者姓名。如有违反上述规定，本网将追究法律责任。", "");
		...
		if(StringUtils.isNotBlank(newsSource)){
			if(newsSource.contains("来源"))
				ne.setPage_source(newsSource.replace(" ", "").replace("来源:", "").replace("来源：", ""));
		}
